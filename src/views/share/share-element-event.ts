...
import { shareState } from './share-config'
import PublishedApi from '@/api/publish'

export const useShareElementEvent = () => {
  /**
   * 鼠标滑过显示要素名称
   */
  const onElementText = (name: string, ele: any) => {
    shareState.text = new mapState.AMap.Text({
      text: name,
    ...
    shareState.text.setExtData({
      elementId: ele.getExtData().elementId
    })
    switch (ele.className) {
      case 'AMap.Marker':
        shareState.text.setPosition(ele.getPosition())
        shareState.text.setOffset(new mapState.AMap.Pixel(40, -15))
        shareState.text.setMap(mapState.map)
        break
      case 'Overlay.Polyline':
        const polylineIndex = Math.floor(ele.getPath().length / 2)
        shareState.text.setPosition(ele.getPath()[polylineIndex])
        shareState.text.setMap(mapState.map)
        break
      case 'Overlay.Circle':
        shareState.text.setPosition(ele.getCenter())
        shareState.text.setMap(mapState.map)
        break
      case 'Overlay.Ellipse':
        shareState.text.setPosition(ele.getCenter())
        shareState.text.setMap(mapState.map, ele.getCenter())
        break
      case 'Overlay.Rectangle':
        ...
        shareState.text.setPosition(bounds)
        shareState.text.setMap(mapState.map)
        break
      case 'Overlay.Polygon':
        const PolygonIndex = Math.floor(ele.getPath().length / 2)
        shareState.text.setPosition(ele.getPath()[PolygonIndex])
        shareState.text.setMap(mapState.map)
        break
      default:
        return
   ...
   * 请求要素信息
   */
  const shareMapElementInquiry = async (id: string, ele: any) => {
    // 请求接口
    const param = {
      id: id,
      encryptId: mapEditState.encryptId
    }
    const res = await PublishedApi.publishedMapElementInquiry(param)
  ...
  const elementInfoWindow = (ele: any) => {
    shareState.infoWindow = new mapState.AMap.InfoWindow({
    ...
    switch (ele.className) {
      case 'AMap.Marker':
        shareState.infoWindow.open(mapState.map, ele.getPosition())
        break
      case 'Overlay.Polyline':
        const polylineIndex = Math.floor(ele.getPath().length / 2)
        shareState.infoWindow.open(mapState.map, ele.getPath()[polylineIndex])
        break
      case 'Overlay.Circle':
        shareState.infoWindow.open(mapState.map, ele.getCenter())
        break
      case 'Overlay.Ellipse':
        shareState.infoWindow.open(mapState.map, ele.getCenter())
        break
      case 'Overlay.Rectangle':
        ...
        shareState.infoWindow.open(mapState.map, bounds)
        break
      case 'Overlay.Polygon':
        const PolygonIndex = Math.floor(ele.getPath().length / 2)
        shareState.infoWindow.open(mapState.map, ele.getPath()[PolygonIndex])
        break
      default:
        return
    }
  ...
  ;(window as any).onPrevPage = (index: number) => {
    ...
    index--
    shareState.infoWindow.setContent(createInfoWindow(index))
  }
  ...
  ;(window as any).onPreview = (index: number) => {
    shareState.isPreview = true
    shareState.index = index
  }
  ...
  ;(window as any).closeInfoWindow = () => {
    shareState.infoWindow.close()
  }

  return {
    onElementText,
    shareMapElementInquiry,
    onCloseViewer
  }
}
