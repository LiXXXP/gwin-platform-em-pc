...
import { useShareElementEvent } from './share-element-event'

const { getElementSum } = useElementEvent()
const { onElementText, shareMapElementInquiry } = useShareElementEvent()

export const useShareElementAdd = () => {
  /**
   * 地图分享页-地图图层要素列表查询
   */
  const publishedMapInquiry = async () => {
    const param = {
      encryptId: mapEditState.encryptId
    }
    const res = await PublishedApi.publishedMapInquiry(param)
    if (res.status.replyCode === '102') {
      router.push('/not')
    }

    // 业务图层列表
    mapEditState.mapElementList = res.body.layerElementList
    mapEditState.mapBasic.mapName = res.body.name
    mapState.map.setMapStyle('amap://styles/' + res.body?.templateNo)
  ...
  const addMarker = (ele: ElementListParam) => {
    ...
    if (ele.display) {
      ...
      // 鼠标移出
      marker.on('mouseout', () => {
        shareState.text.hide()
      })
    }

    // 点击当前点标记
    marker.on('click', async () => {
      await shareMapElementInquiry(ele.elementId, marker)
    })
  }
  ...
  const addPolyline = (ele: ElementListParam) => {
    ...
    polyline.on('click', async () => {
      await shareMapElementInquiry(ele.elementId, polyline)
    })
  }
  ...
  const addCircle = (ele: ElementListParam) => {
    ...
    circle.on('click', async () => {
      await shareMapElementInquiry(ele.elementId, circle)
    })
  }
  ...
  const addEllipse = (ele: ElementListParam) => {
    ...
    ellipse.on('click', async () => {
      await shareMapElementInquiry(ele.elementId, ellipse)
    })
  }
  ...
  const addRectangle = (ele: ElementListParam) => {
    ...
    rectangle.on('click', async () => {
      await shareMapElementInquiry(ele.elementId, rectangle)
    })
  }
  ...
  const addPolygon = (ele: ElementListParam) => {
    ...
    polygon.on('click', async () => {
      await shareMapElementInquiry(ele.elementId, polygon)
    })
  }
  return {
    publishedMapInquiry
